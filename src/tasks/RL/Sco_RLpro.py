...
SCO_PREDICTOR_DIR = os.path.join(RL_DIR, "expression_reward", "Sco_predictor")
TFBS_REWARD_DIR = os.path.join(RL_DIR, "tfbs_reward")

if SCO_PREDICTOR_DIR not in sys.path:
    sys.path.insert(0, SCO_PREDICTOR_DIR)
if TFBS_REWARD_DIR not in sys.path:
    sys.path.insert(0, TFBS_REWARD_DIR)

from sco_predictor import load_activity_predictor, predict_activity_batch
from motifs_fimo import read_meme, scan


# Paths
POLICY_DIR       = "outputs/Scoli/base"
PREDICTOR_DIR    = os.path.join(SCO_PREDICTOR_DIR, "predictor_result")

MEME_PATH        = "datasets/Scoli/Scoli_meme.txt"
GLOBAL_VEC_CSV   = "datasets/Scoli/highscoli_nature_tfbs.csv"

RL_OUT_ROOT   = "outputs/Scoli/RL_base"
...
def save_best_run_config(path: str, ppo_cfg: PPOConfig):
    cfg = {
        ...
        "predictor": {
            ...
            "PRED_MAX_LENGTH": PRED_MAX_LENGTH,
        },
    }
...
# Reward resources
activity_model, activity_tokenizer, activity_device = load_activity_predictor(PREDICTOR_DIR)
print(f"[CHK] activity predictor loaded: {PREDICTOR_DIR}")
print(f"[CHK] activity predictor device: {activity_device}")
...
def reward_a_activity(seqs: List[str]) -> torch.Tensor:
    ...
    seqs = [_to_168_then_165(s) for s in seqs]

    vals = predict_activity_batch(
        model=activity_model,
        tokenizer=activity_tokenizer,
...
def reward_d_base_types(seqs: List[str]) -> torch.Tensor:
    """
    基于 unique-base 阈值的 collapse score（用于约束版）

    设 u = 序列中出现的不同碱基种类数，u ∈ {1,2,3,4}
    先定义退化惩罚：
        collapse_penalty = max(0, 3-u) / 2
    再转成“越大越好”的 score：
        r_d = 1 - collapse_penalty

    因此：
        u=1 -> penalty=1.0 -> r_d=0.0
        u=2 -> penalty=0.5 -> r_d=0.5
        u=3 -> penalty=0.0 -> r_d=1.0
        u=4 -> penalty=0.0 -> r_d=1.0
    """
    scores = []
    for s in seqs:
        s = _to_168_then_165(s)
...
def main():
    ...
    try:
        test_vals = reward_a_activity(["ACGT" * 42, "TTGACA" * 28])
        print(f"[PREDICTOR SMOKE TEST] passed, preds={test_vals.tolist()}")
    except Exception as e:
